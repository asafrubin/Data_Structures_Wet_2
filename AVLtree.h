...
public:
    ...
    AVLnode<DataType, SumFunction>* findMax(AVLnode<DataType, SumFunction>* root){
        if(root == NULL){
            return NULL;
        }
    ...
    void mergeTrees(AVLtree<DataType, CompareFunction, SumFunction>& tree, Func func){
        ...
        mergedSize = merge2array(arrayTree1, array1size, arrayTree2, array2size, mergedArray, func);
    ...
    template<typename Function>
     int merge2array(DataType* arrayTree1, int array1size, DataType* arrayTree2, int array2size, DataType* mergedArray, Function &f){
        int i = 0, j = 0, k = 0;
        while( i < array1size && j < array2size ){
            ...
            else if(compare(arrayTree1[i], arrayTree2[j]) == 0){
                mergedArray[k] = f(arrayTree1[i], arrayTree2[j], k);
                i++;
                j++;
                k++;
            }
