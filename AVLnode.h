...
public:
    AVLnode() : bf(0), hight(0), rank(0), sum(0), data(), l_child(NULL), r_child(NULL), parent(NULL) {};
    explicit AVLnode(DataType data) : bf(0), hight(0), rank(1),sum(data), data(data), l_child(NULL), r_child(NULL), parent(NULL){};
    ~AVLnode(){
        //delete data;
    };
    ...
    */
    ...
    void calcSum(){
        DataType sum_l_child, sum_r_child;
        if(l_child)
            sum_l_child = l_child->getSum();
        if(r_child)
            sum_r_child = r_child->getSum();

        sum = sumFunction(this->getData(), sum_l_child, sum_r_child);
    }
